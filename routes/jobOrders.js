...
router.get('/', authenticateToken, (req, res) => {
    ...
    let query = `
        SELECT jo.*, po.po_number, po.so_number, po.po_date, po.created_at as po_created_at, po.client_id, c.company_name, c.contact_person, c.is_vyuceutical_ops,
               ...
               (SELECT dr.dr_number FROM delivery_items di JOIN delivery_receipts dr ON di.dr_id = dr.id WHERE di.batch_id = (SELECT id FROM production_batches WHERE jo_id = jo.id ORDER BY created_at DESC LIMIT 1) LIMIT 1) as latest_dr_number,
               (SELECT dr.status FROM delivery_items di JOIN delivery_receipts dr ON di.dr_id = dr.id WHERE di.batch_id = (SELECT id FROM production_batches WHERE jo_id = jo.id ORDER BY created_at DESC LIMIT 1) LIMIT 1) as latest_dr_status
        FROM job_orders jo
